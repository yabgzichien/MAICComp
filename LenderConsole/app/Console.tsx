  ...
  return (
    <div style={{ margin: '12px 20px 0', borderRadius: 10, border: `1px solid ${p.hairline}`, background: p.surface2, padding: '10px 12px', flexShrink: 0 }}>
      ...
      {app.status === 'referred' && (
        <div>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button disabled={!canResolve} onClick={() => onResolve('declined', rationale)} style={{ flex: 1, padding: '7px 0', borderRadius: 7, border: `1.5px solid ${canResolve ? p.red : p.hairline}`, cursor: canResolve ? 'pointer' : 'not-allowed', background: 'transparent', color: canResolve ? p.red : p.ink3, fontFamily: FONT.ui, fontSize: 12, fontWeight: 700 }}>Decline</button>
          </div>
        </div>
      )}
      ...
      {app.status === 'approved' && (
        <div>
          {rationaleInput}
          <button disabled={!canResolve} onClick={() => onResolve('declined', rationale)} style={{ width: '100%', padding: '7px 0', borderRadius: 7, border: `1.5px solid ${canResolve ? p.red : p.hairline}`, cursor: canResolve ? 'pointer' : 'not-allowed', background: 'transparent', color: canResolve ? p.red : p.ink3, fontFamily: FONT.ui, fontSize: 12, fontWeight: 700 }}>Overturn to decline</button>
        </div>
      )}
